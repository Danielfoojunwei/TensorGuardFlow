...
class RobustnessBench:
    ...
    def run_byzantine_test(self):
        ...
        for i in range(3):
            ...
            data = good_update + noise
            
            # Create Pkg
            pkg = UpdatePackage(
                client_id=f"good_{i}", 
                delta_tensors={"grad": data.tobytes()}
            )
            agg.add_contribution(ClientContribution(f"good_{i}", pkg, datetime.utcnow()))
            
        # Bad Client 1: Sign Flip
        bad_data_1 = -1 * good_update * 5 # Inverted and Scaled
        pkg1 = UpdatePackage(client_id="bad_flip", delta_tensors={"grad": bad_data_1.tobytes()})
        agg.add_contribution(ClientContribution("bad_flip", pkg1, datetime.utcnow()))
        
        # Bad Client 2: Random Noise injection
        bad_data_2 = np.random.randn(100).astype(np.float32) * 10
        pkg2 = UpdatePackage(client_id="bad_noise", delta_tensors={"grad": bad_data_2.tobytes()})
